...
\vspace{-10pt}
\begin{tabbed}
(Inclusive bounds. If represented by a unit step function, the bounds are inclusive for filters) % Refer to lecture notes
\end{tabbed}
...
\begin{multicols*}{3}
    ...
    Gaussian Integral\\
    \tab $\displaystyle \int_{-\infty}^{\infty} e^{-a(x+b)^2} dx = \sqrt{\frac{\pi}{a}}$\\
    Geometric Series\\
    \tab $\ds \sum_{k=0}^{n}x^k = \frac{1-x^{n+1}}{1-x} \quad\xrightarrow{n\rightarrow\infty \text{ and } x < 1}\quad \frac{1}{1-x}$\\
    Complex Exponential Representations of Sine and Cosine\\
    \tab $\ds \cos\theta = \frac{e^{j\theta}+e^{-j\theta}}{2}$\\
    \tab $\ds \sin\theta = \frac{e^{j\theta}-e^{-j\theta}}{2j}$\\
\end{multicols*}
